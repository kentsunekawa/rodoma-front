...
const Component: React.FC<Props> = (props: Props) => (
  <div className={`${CLASSNAME} ${props.className}`}>
    {props.post ? <PostCreator post={props.post} /> : <LoadingBlock />}
  </div>
);
...
const Container: React.FC<ComponentProps> = (componentProps) => {
  ...
  const emptyPost = (user: UserData_overview): PostData => {
    return {
      id: 0,
      user_id: user.id,
      category_id: 1,
  ...
  const getPost = async (id: number) => {
    try {
      ...
      if (result.status === 'success_get_post' && result.data) {
        if (user) {
          if (user.id !== result.data.post.user_id) {
  ...
  return (
    <PostEditContext.Provider value={{ state, contextDispatch }}>
